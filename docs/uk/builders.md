...
Блок, який ви передаєте, **заміщає** той, що тримає реєстр. Їх не зливають поле за полем, тож
аргумент, якого ви не передали, не надсилається — і реєстр видаляє те, що тримав. Аргумент,
переданий як `""`, надсилається порожнім, і саме це очищає поле.

RFC 5733 можна прочитати як «не подавайте — і реєстр збереже те, що має», бо кожна складова
...
Саме тому `name`, `city` і `country_code` обов'язкові в кожній зміні адреси, і білдер без них
відмовляє. Вони тримають кадр валідним, але не здатні повернути аргумент, якого ви не передали.
**Спершу прочитайте блок і поверніть його разом зі своєю зміною:**

```python
current = client.contact.info("C1").postal_info()["int"]

# Прибрати організацію, лишивши решту адреси точно такою, як була.
(client.contact.update_builder("C1")
