...
Блок, который вы передаёте, **замещает** тот, что хранит реестр. Их не сливают поле за полем, поэтому
аргумент, которого вы не передали, не отправляется — и реестр удаляет то, что хранил. Аргумент,
переданный как `""`, отправляется пустым, и именно это очищает поле.

RFC 5733 можно прочитать как «не передавайте — и реестр сохранит то, что хранит», ведь каждая
...
Именно поэтому `name`, `city` и `country_code` обязательны в любом изменении адреса, и билдер без
них отказывает. Они удерживают кадр валидным, но вернуть аргумент, которого вы не передали, не
могут. **Сначала прочитайте блок и верните его вместе со своим изменением:**

```python
current = client.contact.info("C1").postal_info()["int"]

# Убрать организацию, оставив остальной адрес ровно таким, каким он был.
(client.contact.update_builder("C1")
...
Изменение одной почтовой формы оставляет другую ровно такой, какой она была: они адресуются
раздельно.
